---
id: getting-started
title: Getting started with @maxbezs/boost
sidebar_label: '@maxbezs/boost'
---

The `@accounts` packages are modular by nature and can be manually installed and configured, however we provide `@maxbezs/boost` - a package containing useful abstractions to get a GraphQL accounts server started with minimal configuration.

**Install the core**

```bash
// Npm
npm install --save @maxbezs/boost

// Yarn
yarn add @maxbezs/boost
```
...
```bash
// Npm
npm install --save @maxbezs/mongo
...
```bash
// Npm
npm install --save @maxbezs/password
...
Assuming you've installed the following packages, `@maxbezs/mongo` and `@maxbezs/password` the default mongo connection options will be applied and a database called `accounts-js` will be used.

Out of the box `@maxbezs/password` is preconfigured to allow users to sign up with usernames or email addresses.
...
```javascript
import accountsBoost, { authenticated } from '@maxbezs/boost';
...
(async () => {
  const accounts = (
    await accountsBoost({
      tokenSecret: 'terrible secret',
      micro: true, // setting micro to true will instruct `@maxbezs/boost` to only verify access tokens without any additional session logic
    })
  ).graphql();
