...
exports.runDatabaseTests = void 0;
const crypto_1 = require("crypto");
const generateRandomToken = (length = 43) => (0, crypto_1.randomBytes)(length).toString('hex');
const generatePseudoRandomUuid = () => [4, 2, 2, 2, 6].map((len) => generateRandomToken(len)).join('-');
function delay(time) {
    return new Promise((resolve) => setTimeout(() => resolve(), time));
}
// eslint-disable-next-line jest/no-export
const runDatabaseTests = (tests) => {
    describe('@maxbezs/database-tests', () => {
        ...
        describe('sessions', () => {
            ...
            describe('findSessionById', () => {
                ...
                it('should find the session', async () => {
                    const sessionId = await tests.database.createSession(session.userId, token1, connectionInfo);
                    const resultSession = await tests.database.findSessionById(sessionId);
                    expect(resultSession).toBeTruthy();
                });
            });
            describe('updateSession', () => {
                it('should not throw for not found session', async () => {
                    await expect(tests.database.updateSession(tokenSessionId1, connectionInfo)).resolves.not.toThrow();
                });
            ...
            describe('invalidateAllSessions', () => {
                it('invalidates all sessions', async () => {
                    const sessionId1 = await tests.database.createSession(session.userId, token1, connectionInfo);
                    const sessionId2 = await tests.database.createSession(session.userId, token2, connectionInfo);
                    await delay(10);
                ...
                it('invalidates all sessions except given list', async () => {
                    const sessionId1 = await tests.database.createSession(session.userId, token1, connectionInfo);
                    const sessionId2 = await tests.database.createSession(session.userId, token2, connectionInfo);
                    const sessionId3 = await tests.database.createSession(session.userId, token3, connectionInfo);
                    const sessionId4 = await tests.database.createSession(session.userId, token4, connectionInfo);
                    await delay(10);
